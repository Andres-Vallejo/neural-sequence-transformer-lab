...
def main():
    ...
    _, val_loader, vocab, label_map = get_dataloaders(**cfg["data"])
    ...
    pd.DataFrame(confusion_matrix(y_true, y_pred), index=labels, columns=labels).to_csv(metrics_dir / f"{args.model}_confusion_matrix.csv")
